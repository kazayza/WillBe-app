...
class SmartSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final safeChangePercent =
        analysis.changePercent.isFinite ? analysis.changePercent : 0.0;
              ...
              children: [
                Expanded(
                  child: _buildQuickStat(
                    ...
                    label: 'معدل النمو',
                    value:
                        '${safeChangePercent >= 0 ? '+' : ''}${safeChangePercent.toStringAsFixed(1)}%',
                    valueColor: safeChangePercent >= 0
                        ? Colors.greenAccent
                        : Colors.redAccent,
                  ),
                ),
  ...
  // ═══════════════════════════════════════════════════════════
  // 📅 نص المقارنة مع الفترة
  // ═══════════════════════════════════════════════════════════
  String _getComparisonText() {
    final dateFormat = DateFormat('MM/dd');

    try {
      if (compareWith == 'lastYear') {
        return 'مقارنة بنفس الفترة من ${fromDate.year - 1}';
      } else {
        final duration = toDate.difference(fromDate);
        final prevFrom = fromDate.subtract(duration + const Duration(days: 1));
        final prevTo = fromDate.subtract(const Duration(days: 1));
        return 'مقارنة بـ ${dateFormat.format(prevFrom)} - ${dateFormat.format(prevTo)}';
      }
    } catch (e) {
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            child: Icon(recIcon, color: priorityColor, size: 16),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
  ...
  Widget _buildPrediction(
      PredictionData prediction, NumberFormat currencyFormat) {
    // حماية من القيم غير الصالحة
    final safeProjectedAmount = prediction.projectedAmount.isFinite
        ? prediction.projectedAmount
        : 0.0;
      ...
      children: [
        ...
        Row(
          children: [
            Flexible(
              child: Text(
                '${currencyFormat.format(safeProjectedAmount.round())} ج.م',
                style: const TextStyle(
                  color: Colors.white,
                  fontSize: 24,
                  fontWeight: FontWeight.bold,
            ...
            Container(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.insights, size: 14, color: confidenceColor),
                  const SizedBox(width: 4),
                  Text(
                    confidenceText,
                    style: TextStyle(
